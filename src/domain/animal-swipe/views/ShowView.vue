...
const { category_id, id } = toRefs(props)
...
watch(() => localeStore.activeLocaleCode, () => {
...
<template>
  ...
  <div :class="twMerge('w-screen h-screen brightness-90 duration-300 transition ease-in-out flex flex-col justify-between', forceLandscape)" :style="bgImage">
    <div class="flex justify-between items-start text-white" :style="outlineText">
      <RouterLink :to="{ name: 'animal-swipe' }" class="inline-flex items-center text-white font-bold gap-1">
      ...
      <ul class="bg-amber-700 p-3 rounded-bl-md flex gap-4">
        ...
        <li class="rounded-full p-1 bg-amber-600 flex items-center" @click="localeStore.setNextLocale">
    ...
    <div class="self-center relative bottom-8 flex gap-8 text-white items-center">
      <h1 class="text-3xl self-centerfont-bold font-poppins" :style="outlineText">{{ store.animal?.name }}</h1>
    </div>
  </div>
</template>
